...
model = 'selfrag/self_rag_critic'
model_short = 'selfrag-critic-official'
use_trigger_template = False
...
sampling_params = SamplingParams(temperature=0.0, top_p=1.0, max_tokens=10, logprobs=len(tokenizer))
...
with open(out_file, 'w') as out_f, open(out_file_greedy, 'w') as out_f_greedy:
    for entry in tqdm(critic_task2data['retrieval']):
        ...
        print(PROMPT_DICT['prompt_input'].format_map(entry))
        print(preds[0].outputs[0].text)
        print('\n\n+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n\n')

        cur_pred = preds[0].outputs[0].text.strip().lower().lstrip('[').rstrip(']')
        cur_gt = entry['output'].lower().lstrip('[').rstrip(']')
        if cur_pred == cur_gt:
            acc.append(1)
        else:
            acc.append(0)
        all_preds.append(cur_pred if cur_pred in ['retrieval', 'no retrieval'] else 'other')
        print(Counter(all_preds), np.mean(acc))
