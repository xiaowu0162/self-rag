...
def call_model_beam_batch(prompt, model, max_new_tokens=15, ctxs=None, query=None, max_depth=5, rel_tokens=None,
    ...
    # print(ret_token_score_dict["[Retrieval]"], ret_token_score_dict["[No Retrieval]"])
    # print('First round retrieve prob:', retrieve_prob)
    first_round_retrieve_prob = retrieve_prob
    
    if threshold is None:
        do_retrieve = "[Retrieval]" in preds[0]
    else:
        do_retrieve = True if retrieve_prob > threshold else False
    retrieval_triggered_at_start = do_retrieve
    ...
    if do_retrieve is False:
        ...
        results = {
            ...
            "depth": None,
            "first_round_retrieve_prob": first_round_retrieve_prob,
            "retrieval_triggered_at_start": retrieval_triggered_at_start
        }
        ...
        prediction_tree = {}
        levels = {}
        prediction_tree[node_id] = {"prompt": prompt, "pred": "[Retrieval]", "retrieve_prob": retrieve_prob,
    ...
    try:
        x = first_round_retrieve_prob
    except:
        first_round_retrieve_prob = None
    ...
    # print(original_splitted_sentences)
...
def main():
    ...
    # dummy args to unify with short-form gen
    ...
    print(f'Note: args.metric={args.metric} is ignored.')
    # print(f'Note: args.store_retrieval_prob is ignored.')
    ...
    if args.task is not None and args.task == "factscore":
        ...
        for idx, item in tqdm(enumerate(input_data)):
            ...
            if type(result) == str:
                postprocessed_result = fix_spacing(postprocess(result))
            else:
                postprocessed_result = fix_spacing(postprocess(result[0]))
                
            # logs for analysis
            print(f'\tret={intermediate["retrieval_triggered_at_start"]}, ret_prob={intermediate["first_round_retrieve_prob"]:.04f}, depth={intermediate["depth"]}', flush=True)
            new_results.append({"input": item["input"], "output": postprocessed_result, "topic": item["topic"],
                                "cat": item["cat"], 
                                "intermediate": intermediate["original_splitted_sentences"][0] if intermediate and "original_splitted_sentences" in intermediate else None,
                                "first_round_retrieve_prob": intermediate["first_round_retrieve_prob"],
                                "retrieval_triggered_at_start": intermediate["retrieval_triggered_at_start"],
        ...
        for instance_idx, item in tqdm(enumerate(input_data)):
            ...
            # logs for analysis
            item["first_round_retrieve_prob"] = intermediate["first_round_retrieve_prob"]
            item["depth"] = intermediate["depth"]
            item["retrieval_triggered_at_start"] = intermediate["retrieval_triggered_at_start"]
            
            print(f'\tret={item["retrieval_triggered_at_start"]}, ret_prob={item["first_round_retrieve_prob"]:.04f}, depth={item["depth"]}', flush=True)
